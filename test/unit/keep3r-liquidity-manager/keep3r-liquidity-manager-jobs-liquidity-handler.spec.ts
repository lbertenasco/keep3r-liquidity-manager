  ...
  describe('addJob', () => {
    when('doenst exist in jobs', () => {
      given(async function () {
        ...
        this.addJobTx = keep3rLiquidityManagerJobsLiquditiyHandler.addJob(
          this.jobAddress
        );
        await this.addJobTx;
      });
      then('adds it', async function () {
        expect(
          await keep3rLiquidityManagerJobsLiquditiyHandler.jobs()
        ).to.deep.equal([this.jobAddress]);
      });
      then('emits event', async function () {
        await expect(this.addJobTx)
          .to.emit(keep3rLiquidityManagerJobsLiquditiyHandler, 'JobAdded')
          .withArgs(this.jobAddress);
      });
    });
    when('exists in jobs', () => {
      given(async function () {
        this.jobAddress = await wallet.generateRandomAddress();
        await keep3rLiquidityManagerJobsLiquditiyHandler.addJob(
          this.jobAddress
        );
        this.addJobTx = keep3rLiquidityManagerJobsLiquditiyHandler.addJob(
          this.jobAddress
  ...
  describe('removeJob', () => {
    ...
    when('doenst exist in jobs', () => {
      given(async function () {
        ...
        this.removeJobTx = keep3rLiquidityManagerJobsLiquditiyHandler.removeJob(
          this.jobAddress2
        );
      ...
      then('event is not emitted', async function () {
        await eventUtils.expectNoEventWithName(
          await this.removeJobTx,
          'JobRemoved'
        );
      });
    ...
    when('exists in jobs', () => {
      ...
      then('emits event', async function () {
        await expect(this.removeJobTx)
          .to.emit(keep3rLiquidityManagerJobsLiquditiyHandler, 'JobRemoved')
          .withArgs(this.jobAddress);
      });
    });
  });
